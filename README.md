...
本项目是一个基于 Next.js 构建的运动姿态分析应用。用户可以上传运动视频，应用通过 LiteLLM 聚合层访问多家大模型对视频中的运动姿态进行分析，并提供反馈。

## 核心业务流程

1.  **视频上传**: 用户在客户端选择运动视频文件。应用后端会生成一个预签名的 AWS S3 上传链接，用户通过此链接将视频直接上传到 S3 存储桶。
2.  **分析请求提交**: 视频上传完成后，用户提交分析请求。后端服务会记录此请求，并创建一个分析任务。
3.  **姿态分析**: 后端服务通过 LiteLLM 调用所配置的模型（例如 Gemini、GPT-4o 等），将存储在 S3 中的视频作为输入进行姿态分析。
